...
When I think about software, I like designing software so that doing the right things are easy and doing wrong things are impossible (or at least very hard). This is typically referred to as [falling into the pit of success](https://blog.codinghorror.com/falling-into-the-pit-of-success/).

To me, having a well defined domain model can prevent many mistakes from happening just because the code _literally_ won't let it happen (either through a compilation error or other mechanisms).

I'm a proponent of functional programming as I believe it allows us to model software in a better way that can reduce the number of errors we make.

In this post, I'm going to talk about one of my favorite techniques, _discriminated unions_.

## Motivation

Let's take a look at a real world example where this can happen.
...
Let's go ahead and define the `TimelineEvent` type. One approach we can follow is to start copying the fields from the events in the array. By doing this, we would get the following
...
The main problem with this approach is that the engineer has to remember that the `labeled` event has a `label` property, but not the `locked` property. This might not be a big deal right now, but given that the GitHub API has over [40 event types](https://docs.github.com/en/webhooks-and-events/events/issue-event-types), the odds of forgetting which properties belong where can be challenging. 
...
One of the cool things about Typescript is that there is a [union operator (|)](https://www.typescriptlang.org/docs/handbook/typescript-in-5-minutes-func.html#unions), that allows you to define a type as one of other types.
...
At this point, we have three types, one for each specific event. A `LockedEvent` has no knowledge of a `label` property and a `RenamedEvent` has no knowledge of a a `lock_reason` property.
...
Not only do we not have to use the `!` operator to ignore type safety, we have better autocomplete as well (note that `locked_reason` and `rename` don't appear when working with a labeled event).
![Better autocomplete](../images/typescript-union-intellisense.png)


## Deeper Dive

At a general level, this concept is known as a [sum type](https://en.wikipedia.org/wiki/Tagged_union) and it's great for when you have a type that can take on a finite number of differing shapes.

When it comes to sum types, you can have both _tagged unions_ and _untagged unions_. Typescript has untagged unions, however, other languages like Haskell and F#, use tagged unions. Let's see what the same implementation in F# would have looked like this
...
In this case, a _tagged union_ is the constructor that is used to create the specific value. In the Typescript example, we worked around it because the `event` property is on every `TimelineEvent` and is a different value. 
...
When defining domain models, you can find yourself needing to model a type that can take on different shapes. If you know that there's a finite number of shapes, then using a `sum` type can help model the domain in a type safe fashion.